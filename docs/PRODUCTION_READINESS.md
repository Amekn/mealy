...
- Status: the exact v0.2.0 onboarding runtime passed its 24-hour gate; protected report CI,
  final free-model OpenRouter acceptance, attested publication, and public repository acceptance
  remain
- Reviewed: 2026-07-25
...
Its exact private custom-provider acceptance passed, and the repository signing identity is
configured and clean-install tested. The report-bearing commit still requires protected CI and
reviewed free-model OpenRouter acceptance before the new semantic-version tag can run attested
multi-distribution publication plus dependent public-download verification.
...
runtime now has a passing 24-hour report; installed-package and supported-distribution
onboarding-to-chat acceptance must still pass before the newer tag can claim public completion.
...
APT, DNF, and Pacman build, signature, tamper, and clean-install acceptance. The exact release still
requires a fully green qualifying tag workflow. Source tests do not retroactively create a public
repository.
...
Owner-local private-provider acceptance passed but remains additional operator evidence and does
not substitute for the final public free-model gate.
