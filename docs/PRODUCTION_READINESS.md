...
- Status: v0.2.1 is the qualified public Linux release; v0.3.0 daily-use implementation and its
  exact-binary 24-hour gate are complete while final release qualification remains active; broader
...
immutable tag and its exact report remain historical. The corrected v0.2.1 release has protected-CI
Lintian parity, its own successful 86,425.487-second exact-binary report, owner-reviewed free-model
live acceptance, 17 digest-bearing attested assets, and signed public repositories. After the
original workflow's checkout-free verifier defect, the remediation passed all 16 protected-main
checks in [run 30324063774](https://github.com/Amekn/mealy/actions/runs/30324063774), and the
protected non-publishing [repository acceptance run
30324688498](https://github.com/Amekn/mealy/actions/runs/30324688498) passed its immutable-release
qualifier plus all five public clean-install lanes.

The exact v0.3.0 daemon at clean revision `d679d04e`, SHA-256
`05d293add133726b07256c8519c04974dd17c3769b1aabdfacb02e12dd24794a`, subsequently completed a
fresh 86,406.720-second retained-disk soak: 20,152 successful turns, 50 hard restarts, 211
interrupted-provider recoveries, 94 undispatched-model resumes, complete recorded-only replay,
SQLite integrity `ok`, clean drain, and zero residual work. Required linear-history rebase maps
that exact observed tree to protected `main` through the checked lineage proof. This is runtime
qualification, not a public-release claim; evidence-only CI, both exact-commit live-provider runs,
the stable tag/attestation workflow, and public distribution acceptance remain mandatory.
...
| Packaging | Versioned Linux packages and archives install without a Rust toolchain; checksums, provenance/SBOM, uninstall, upgrade, and rollback are documented and tested. | A tag-gated matrix natively builds x86-64/ARM64 generic archives, Debian packages, and RPMs plus an x86-64 Arch package from one verified payload. Each format is passive: no maintainer script, service activation, install hook, credential access, or home mutation. Reproducibility tests compare independent package builds byte for byte; inventory, modes, ownership, checksums, version/schema identity, dependency contracts, and tamper rejection are fail closed. Clean pinned images build and exercise packages on Ubuntu 24.04/26.04, Debian 13, Fedora 44, and Arch Linux. The v0.3 candidate also binds an attested public v0.2.1 package baseline and requires session/task/replay/identity preservation, an exact schema-16-to-18 rollback snapshot, new-version metadata writes, and state-preserving removal through DPKG, RPM, and Pacman; local Ubuntu, Fedora, and Arch executions pass, while the protected tag/public-distro repetitions remain pending. The stable archive manager additionally provides locked active/previous slots, safe same-schema rollback, coordinated exact-digest cross-schema home activation, hard-kill compensation, and state-preserving uninstall. Every published asset receives checksums, a CycloneDX SBOM, dependency notice, and offline-verifiable GitHub provenance. The release source also builds signed APT, DNF, and Pacman repositories from those exact packages, attests their complete manifest, and requires clean public HTTPS installs on native x86-64/ARM64 runners. v0.2.1 published all 17 digest-bearing assets and the signed Pages repositories after its builds, packages, attestations, rootless acceptance, and native distribution tests passed. The protected [post-publication run 30324688498](https://github.com/Amekn/mealy/actions/runs/30324688498) independently verified the unchanged release, attestation, and manifest, then clean-installed the exact package through APT on Ubuntu x86-64 and Debian ARM64, DNF on Fedora x86-64/ARM64, and Pacman on Arch x86-64. | P0 acceptance is complete for v0.2.1; the v0.3 package gate is implemented and locally green but not complete until protected tag and public-distribution jobs repeat it. The original run's five repository jobs remain transparent negative evidence of a checkout-free verifier invocation that failed before reading an artifact; remediation did not change any release or repository content. |
...
`gpt-5.6-sol` recommendation without exposing account or token material. The exact integrated
runtime has a passing 24-hour report, and the published v0.2.1 artifact plus
supported-distribution onboarding-to-chat acceptance passed before final qualification.
...
the integrated source. GitHub Pages and protected deployment environments are activated; the
owner-controlled repository identity and signing subkey are configured, and that exact key passed
APT, DNF, and Pacman build, signature, tamper, and clean-install acceptance. v0.2.1 completed
immutable publication and the protected public acceptance workflow for the unchanged tag,
attestation, signed manifest, and Pages content. A verifier-only failure after publication may be
closed by that protected, non-publishing workflow; any shipped-content failure still requires a
new version. Source tests do not retroactively create a public repository.
...
`7b5d39502e96bbb03c4c33280c6355a91682234d14a5284ded83c143807a55bc`. The preserved
[v0.1.1 report](benchmarks/2026-07-24-v0.1.1-release-soak.json) qualifies revision `8867c467` and
daemon SHA-256 `78591cafdbe79691805d651ecc03e3383313fec8bfeb6ed3428a051fa23f69a7`.
...
but the release-only Debian metadata gate rejected the package before publication. The preserved
[v0.2.1 report](benchmarks/2026-07-27-v0.2.1-release-soak.json) independently qualifies corrected
revision `eec96a8f` and daemon SHA-256
`63068abbc5b270f68c16adcf82b773ffcb64d96c1208012337241280dd480371`. The current canonical
[v0.3 report](benchmarks/release-soak.json) qualifies revision `d679d04e` and daemon SHA-256
`05d293add133726b07256c8519c04974dd17c3769b1aabdfacb02e12dd24794a` through its checked
identical-tree rebase lineage.
...
evidence that motivated the final design, not a replacement for the long-duration gate. The
preserved schema-16
[v0.2.1 release soak](benchmarks/2026-07-27-v0.2.1-release-soak.json) subsequently completed
19,248 turns and 48 hard restarts against the exact packaged daemon with complete replay,
integrity `ok`, and zero residue.
...
the same retained-state burst with a 50-millisecond maximum and bounded memory growth. The corrected
v0.3 subject then completed the canonical 86,406.720-second
[release soak](benchmarks/release-soak.json) from zero with 20,152 successful turns, 50 hard
restarts, complete replay/integrity/drain, and zero residue. The long-duration gate is closed; v0.3
still cannot be called public or production-qualified until its evidence-only protected commit,
dual live-provider acceptance, attested tag workflow, and clean public distribution lanes pass.
