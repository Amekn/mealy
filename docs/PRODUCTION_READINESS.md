...
APT, DNF, and Pacman build, signature, tamper, and clean-install acceptance. An exact release still
requires successful immutable publication plus green public acceptance. A verifier-only failure
after publication may be closed by the protected, non-publishing post-publication workflow bound to
the unchanged tag commit and original attestation; any shipped-content failure requires a new
version. Source tests do not retroactively create a public repository.
