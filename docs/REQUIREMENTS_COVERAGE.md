...
| AUTO-010..020 | covered | Schema 29 keeps recurring cron stable while adding owner-keyed future one-shot prompts/notifications and exact future direct-session-event notifications. Canonical UUIDv7 creation is replay-safe after due time, cursor movement, and lifecycle advancement; semantic reuse conflicts. Whole-definition revisions, exclusive create/edit/resume cursors, 30-second leased occurrences, deterministic prompt inbox keys, atomic notification outbox/run/cursor/journal completion, terminal one-shots, poison-event advancement, same-principal exact bindings, safe-mode denial, and active/paused/claimed/failed operator gauges are enforced. Event rules are static notification-only and never copy payloads. Schema 30 permits proactive Slack static notifications only through one UUIDv7-keyed, expiring route to an exact previously admitted owner thread. Definitions pin that ID; create/edit, outbox publication, and delivery claim revalidate it without newest-thread or local-delivery fallback. Store/migration units plus the real `phase7_operations` create/retry/pause/resume/drain/hard-restart scenario and `slack_channel` restart/pin/delivery/revoke scenario provide evidence. |
...
| MEM-001, MEM-010..015 | covered | Governed proposal/activation/rejection/supersession/expiry/deletion, provenance/namespace/confidence/sensitivity/retention, deterministic FTS5, optional privacy-scoped semantic retrieval, safe lexical fallback, untrusted citations, correction/pin/export/index rebuild. The chat-native and scriptable `remember` workflows preserve distinct proposal/owner-approval commits, generated exact-content provenance, optimistic revision fences, and recoverable partial-failure IDs. Schema 28 keeps vectors derived, complete-set/config/revision/content fenced, principal scoped, lifecycle invalidated, and non-searchable while stale/degraded/incompatible. Exact cosine plus reciprocal-rank fusion reports actual retrieval mode/status and never replaces canonical cited memory. `phase5_memory_context`, `memory_workflow`, adapter/config/parser, PTY, and memory store tests include cross-scope denial, lifecycle UX, local endpoint calls, hybrid recall, hard-restart fallback/recovery, and tombstones. |
...
Schema 28 extends MEM/CFG/DATA/NFR-REL/NFR-QUAL evidence without creating another memory
authority. The stopped-home policy pins an exact OpenAI-compatible model, dimensions, prefixes,
residency, timeout, and credential reference; remote transport requires HTTPS, while
literal-loopback HTTP may be credentialless. The adapter ignores proxies and redirects, bounds
request/response/numeric behavior, and confines its blocking client plus zeroizing credential to a
dedicated bounded worker lifetime. Complete-set rebuild performs network work outside the writer
and atomically rechecks active revision/content/configuration fences. SQLite triggers remove
affected vectors and persist stale health on lifecycle drift. Search applies canonical scope,
status, sensitivity, and digest filters before exact cosine plus deterministic reciprocal-rank
fusion; every unavailable or invalid state reports a fixed classification and returns lexical
fallback. Adapter/config/store units and the real `phase5_memory_context` process proof cover
prefix/model identity, dimensions, secret-free errors, owner isolation, ranking, correction,
deletion, hard restart, stale fallback, and rebuild recovery.

Schema 29 extends AUTO/SCHED/CHAN/REC/OBS/NFR-REL/NFR-QUAL evidence without turning journal events
into an execution authority. Revisioned definitions retain immutable definition digests and an
exclusive future-event cursor. SQLite constraints/triggers enforce ownership, action/trigger
shape, transition fences, immutable run identity, terminal outcome shape, and bounded claim
recovery. The daemon scans a bounded batch, admits prompts only through the ordinary session use
case, and emits only static payload-free event notifications through supported existing outbox
routes. Migration, store, CLI/API, process restart, safe-mode, package inventory, status/metrics,
and documentation validation cover the public contract.

Schema 30 extends AUTO/CHAN/AUTH/REC/NFR-REL/NFR-QUAL evidence with outbound-only exact-thread Slack
continuation. Creation requires a client-proposed UUIDv7, active exact owner binding, admitted
workspace/member/conversation/session/thread receipt, bounded expiry, and no overlapping effective
pin; it atomically records a no-replay cursor and immutable source evidence. Static notification
definitions retain the exact route ID. SQLite and the daemon revalidate it at definition mutation,
outbox publication, and delivery claim, so expiry/revocation/mismatch cannot switch threads or
fall through to local delivery. Store tests cover exact retry, overlap, expiry and revocation; the
real Slack process fixture covers crash recovery, API pin/list/read, exact-thread proactive
delivery, revocation, and post-revocation rejection.
...
provider, session-workbench/input, task-control/replay, durable-delegation inspection, approval,
automation, extension, and channel operations. It rejects ambiguous origins and identifiers,
disallows remote clear-text transport, ambient proxies, and redirects, redacts bearer material,
bounds request/response bytes, validates request/response/error versions, and preserves structured
retry evidence. Real loopback unit fixtures cover exact headers, paths, queries, bodies, receipts,
size/version failures, private-descriptor validation, and debug redaction. Frozen
v0.2.1/v0.3/v0.4/v0.5 daemon exchanges prove cross-release decoding, while reproducible
publishable crate archives, an exact consumer lock, and clean extracted-package compilation are
enforced in CI, tagged publication, and downloaded-public acceptance. Async/SSE and non-Rust
bindings remain outside the v0.5 stable blocking SDK contract.
