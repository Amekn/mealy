...
| AUTO-010..018 | covered | Schema 29 keeps recurring cron stable while adding owner-keyed future one-shot prompts/notifications and exact future direct-session-event notifications. Canonical UUIDv7 creation is replay-safe after due time, cursor movement, and lifecycle advancement; semantic reuse conflicts. Whole-definition revisions, exclusive create/edit/resume cursors, 30-second leased occurrences, deterministic prompt inbox keys, atomic notification outbox/run/cursor/journal completion, terminal one-shots, poison-event advancement, same-principal exact bindings, safe-mode denial, and active/paused/claimed/failed operator gauges are enforced. Event rules are static notification-only and never copy payloads. Store/migration units plus the real `phase7_operations` create/retry/pause/resume/drain/hard-restart process scenario provide evidence. |
