...
(120,170,857 bytes, SHA-256
`7dd9d23b46fa7a9bfa26f1af96f413e0514c32698f6a43a57e1ade48d88a6578`) after the official
stable-channel metadata moved `151.0.7922.47` to Beta. Checkout `v7.0.1`, artifact upload
`v7.0.1`, artifact download `v8.0.1`, every Pages action, SBOM Action `v0.24.0`, Syft
`v1.50.0`, Cargo Audit `0.22.2`, Cargo Auditable `0.7.5`, Cargo About `0.9.1`, Cargo Deny
`0.20.2`, and zizmor `v1.28.0` remain the reviewed pins. The refresh also advanced Attest to commit
...
the release boundary is unnecessary. The browser-helper, integration-expectation, workflow, and
documentation refresh does not change the soaked Rust binary source boundary; protected CI must
still exercise the updated browser and action before publication.
